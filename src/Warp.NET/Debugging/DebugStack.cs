...
public static class DebugStack
{
	...
	/// </summary>
	/// <param name="name">The name of the debug stack entry.</param>
	...
	public static void Add(string name, object? value, bool clearAfterFrame = false, [CallerFilePath] string filePath = "", [CallerMemberName] string memberName = "", [CallerLineNumber] int lineNumber = 0)
		=> Add(name, value, WarpBase.Game.Dt, clearAfterFrame, GetCallerDeclaration(filePath, memberName), lineNumber);
	...
	/// <param name="memberName">The caller member name (<see cref="CallerMemberNameAttribute"/>).</param>
	/// <param name="lineNumber">The caller line number (<see cref="CallerLineNumberAttribute"/>).</param>
	public static void Add(string name, object? value, float timeout, [CallerFilePath] string filePath = "", [CallerMemberName] string memberName = "", [CallerLineNumber] int lineNumber = 0)
		=> Add(name, value, timeout, false, GetCallerDeclaration(filePath, memberName), lineNumber);
	...
	private sealed class DebugStackEntry
	{
		...
		{
			DebugStackDisplaySetting.Simple => $"{Name,-15} {Value}",
			_ => ToVerboseString(),
		};
